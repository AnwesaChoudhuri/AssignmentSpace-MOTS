...
def k_best_costs(nbest,matrix):
  ...
  best_costs=[]
  best_assignments=[]
  best_global_assignments = []
  best_rule=[]#output array containing costs
  ...
  best_assignments.append(hun)
  best_rule=[[]]

  # Murty's algorithm for finding next k best costs 

  partition_list(matrix,node_rule_list,cost_list,rsrv_list,assignment_list)
  for x in range (0,nbest-1):
    min_cost=min(cost_list)
    best_costs.append(min_cost)
    min_index=cost_list.index(min_cost)
    best_assignments.append(assignment_list[min_index])
    best_rule.append(node_rule_list[min_index])
    partition_list(matrix,node_rule_list,cost_list,rsrv_list,assignment_list)
    cost_list[min_index]=10E6
    best_global_assignments=[]

  for i in range(0,len(best_costs)):
      node_rule=best_rule[i]
      global_assignment=best_assignments[i]
      rule_row=[]
      rule_col = []
      for node in node_rule[::-1]:
          if node[0]>=0:
              rule_row.append(node[0])
              rule_col.append(node[1])
          else:
              if rule_row!=[]:
                global_assignment = get_global_assignment(global_assignment, rule_row, rule_col,len(best_assignments)+len(rule_row),
                                                        len(best_assignments)+len(rule_col))
              rule_row = []
              rule_col = []
      best_global_assignments.append(global_assignment)

  # for i in range(0, len(best_costs)):
  #     print(best_costs[i],matrix[best_global_assignments[i][:,0],best_global_assignments[i][:,1]])
  return best_costs, best_global_assignments


def partition_list(matrix_orig,node_rule_list,cost_list,rsrv_list,assignment_list):
  try:
  ...
  #assignments_global=get_global_assignments(assignments)
...
def get_global_assignment(hun,rows,cols,row_max,col_max):
    temp_matrix=np.zeros((row_max,col_max))
    temp_matrix[rows, cols] = 1 # the already chosen rodes from previous
    col_idx=[]
    row_idx=[]
    for i in range(0,row_max):
        if max(temp_matrix[i, :]) != 0:
            continue
        row_idx.append(i)
    for j in range(0,col_max):
        if max(temp_matrix[:,j])!=0:
            continue
        col_idx.append(j)

    for pair in hun:
        temp_matrix[row_idx[pair[0]],col_idx[pair[1]]]=1

    hun_global=np.transpose(np.concatenate([np.where(temp_matrix>0)[0:1],np.where(temp_matrix>0)[1:]],axis=0))
