...
def plot_img_id(img_id):
    ...
    count = 0
    base = 24000 # 26000 for car, 24000 for person
    not_car = 0
    for ann in anns_:
        if ann['category_id'] in [1, 2]:# [4, 5, 6, 7, 8] for car, [1,2] for pedestrian, check this file: instances_train.json as mentioned in the pointtrack readme
            count += 1
            if ann['category_id'] == 2: # 4 for car, 2 for pedestrian:
...
if __name__ == '__main__':
    ...
    save_path = kittiRoot + "training/KINS_person/"
    ...
    save_path = kittiRoot + "/testing/KINS_person/"
    mkdir_if_no(save_path)
